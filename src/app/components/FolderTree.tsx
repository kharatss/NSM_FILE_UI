...
const FolderTree: React.FC<FolderTreeProps> = ({ data, rightSection }) => {
  return (
    <div className="space-y-2 px-3">
      {data.map((node, index) => (
        <div className="rounded-lg bg-white border border-grey-300" key={index}>
          <TreeNodeComponent key={index} node={node} rightSection={rightSection} />
        </div>
      ))}
    </div>
  );
...
const TreeNodeComponent: React.FC<TreeNodeComponentProps> = ({ node, rightSection = false }) => {
  ...
  return (
    <div >
      ...
      >
        ...
        {rightSection && (
          <div className="ml-4 flex items-center text-gray-700 text-sm space-x-4">
            <p className="truncate w-40">{node.description}</p>
            <p className="w-32">{format(new Date(node.createdAt), "dd-MM-yyyy HH:mm")}</p>
            <p className="w-32">{format(new Date(node.updatedAt), "dd-MM-yyyy HH:mm")}</p>
          </div>
        )}
        {(!rightSection && node.type === "folder") && (
          <div className="ml-4">
            <FontAwesomeIcon
              icon={faPlusCircle}
      ...
      {(isExpanded) && node.children && (
        <div className="pl-6 mt-2 border-l-2 border-gray-200">
          <FolderTree data={node.children} rightSection={true} />
        </div>
      )}
    </div>
  );
